...
class Game {
				...
				for (let k = -1; k < 2; k++) {
					...
					for (let l = -1; l < 2; l++) {
						...
						if (dir.x == dir.y && dir.z == dir.w && dir.w == dir.x && dir.x == 0) {
							...
							if (this.checkrow(pos.clone(), dir.clone())) return true;
						}
					}
				}
			}
	...
	checkrow(pos: Vector4, direction: Vector4): boolean {
		let contents: string[] = [];
		while (pos.x < 4 && pos.y < 4 && pos.z < 4 && pos.w < 4 && pos.x > -1 && pos.y > -1 && pos.z > -1 && pos.w > -1) {
		...
		//durchgehen der Reihe in beide Richtungen
		while (pos.x > -1 && pos.y > -1 && pos.z > -1 && pos.w > -1 && pos.x < 4 && pos.y < 4 && pos.z < 4 && pos.w < 4) {
			contents.push(this.board[pos.x][pos.y][pos.z][pos.w]);
			pos.sub(direction);
		}
